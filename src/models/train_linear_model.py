...
class VAE(keras.Model):
    ...
    Example:
        ...
        vae.compile_and_train(train_data, num_epochs=50, batch_size=64)
    """
    def __init__(self, input_shape, kl_weight, learning_rate, svd_weight, latent_dim=2):
...
def get_labels(model_type, temperature, band_temperature, sparse_temperature):
    if model_type == '1':
        labels = temperature
    elif model_type == '2':
        labels = band_temperature
    elif model_type == '3':
        labels = sparse_temperature
    else:
...
def plot_and_save_latent_space_2D(z_x, z_y, z_T, learning_rate, file_Path, model_name):
    ...
    fig1_name = f'\\2D_B{BATCH_SIZE}_LR{learning_rate_str}_forced_{model_name}.png'
...
def plot_and_save_latent_space_3D(z_x, z_y, z_T, learning_rate, file_Path, model_name):
    ...
    fig2_name = f'\\3D_B{BATCH_SIZE}_LR{learning_rate_str}_forced_{model_name}.png'
...
if __name__ == "__main__":
    ...
    OUTPUT_PATH_WEIGHTS = r"../../../models/weights/forced"
    OUTPUT_PATH_MODEL_DATA = r"../../../models/model_data/forced_model_data_"
    ...
    labels = get_labels(model_type, temperature_number, band_temperature, sparse_temperature)
    ...
    dataset = tf.data.Dataset.from_tensor_slices((model_signal, labels))

    train_dataset = (dataset.shuffle(data_dim).batch(BATCH_SIZE))

    ## Creation of the VAE
    vae = VAE(length_catch, KL_WEIGHT, LEARNING_RATE, SVD_WEIGHT)
